...
export function Message({ onUserClick, content, type, sentAt }: Props) {
  return (
    <div className="flex gap-2">
      <button className="w-8 h-8 rounded-full cursor-pointer" onClick={() => onUserClick('juye')}>
        <UserRound className="w-8 h-8 rounded-full" />
      </button>

      <div className="flex flex-col gap-1">
        <div className="text-sm font-semibold">juye</div>
